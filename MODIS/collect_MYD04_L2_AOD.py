...
PRODUCT = "MYD04_L2"  # Aqua MODIS Dark Target / Deep Blue Aerosol (Level 2, swath)
SERVER = f'https://ladsweb.modaps.eosdis.nasa.gov/archive/allData/61/{PRODUCT}'
DOWNLOAD_DIR = r'/home/slow_data/Air_Quality/MODIS_MYD04_L2'
...
def get_vietnam_granule_names(date_obj):
    """
    Query NASA CMR for MYD04_L2 granule names whose spatial footprint
    ...
    params = {
        'short_name'   : PRODUCT,
        # Do NOT pass 'version' — it causes CMR to return 0 results for MYD04_L2.
        ...
        'page_size'    : 2000,          # MYD04_L2 has ~300 granules/day globally
        'downloadable' : 'true',
    }
...
def filter_hdf(files, vietnam_names=None):
    """
    Keep only .hdf files.
    MYD04_L2 swath files follow the naming pattern:
...
def download_for_date(date_obj):
    """Download MYD04_L2 swath files that cover Vietnam for a specific date."""
