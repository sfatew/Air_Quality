...
yaml_path = os.path.join(grandparent_dir, 'config', 'config.yaml')
...
SERVER = 'https://ladsweb.modaps.eosdis.nasa.gov/archive/allData/61/MOD13Q1'
DOWNLOAD_DIR = r'/home/slow_data/Air_Quality/MODIS_MOD13Q1/raw'
...
def next_composite_date(date_obj):
    """Advance to the next MOD13Q1 composite date: +16 days, reset to Jan 1 on year change."""
    next_date = date_obj + timedelta(days=16)
    if next_date.year != date_obj.year:
...
def check_for_updates(last_date):
    ...
    while True:
        ...
        check_date = next_composite_date(last_checked_date)
        ...
        while check_date <= current_date_eod:
            try:
                ...
                # Composite for this date isn't published yet — try again on next scheduled check.
                print(f"⏭️ No data yet for {check_date.strftime('%Y-%m-%d')}, will retry later.")
                break
            last_checked_date = check_date
            check_date = next_composite_date(check_date)
            dates_downloaded += 1
...
def main():
    ...
    print(f"🔥 Starting MOD13Q1 (Terra VI 16-day) historical download from {start_date.strftime('%Y-%m-%d')} to {current_date_eod.strftime('%Y-%m-%d')}")

    while date <= current_date_eod:
        try:
            download_for_date(date)
            last_downloaded_date = date  # Update only on success
        except FileNotFoundError:
            pass
        date = next_composite_date(date)
